...
// caught).
// `with` on a run_generic_action step fills that library entry's {{param}}
// placeholders for this one use (e.g. a site's own "Next" button selector
// for the generic 'paginate' action), leaving any other placeholders for
// the caller's runtime params. Values may themselves contain {{param}}.
function applyWith(steps, withValues) {
  if (!withValues || typeof withValues !== 'object') return steps;
  ...
  for (const [key, value] of Object.entries(withValues)) {
    ...
    json = json.split(`{{${key}}}`).join(escaped);
  }
  return JSON.parse(json);
}

// Expansion inlines a referenced action's steps into the caller's list,
// which loses the one fact you most want when the run breaks: that the
// failing step came from `dismiss_overlay` rather than from the recipe
// itself. `_from` carries that through as a breadcrumb ("paginate >
// scroll_to_end"), read back by engine.js when reporting a failed step.
// Purely descriptive — nothing dispatches on it.
function tagOrigin(steps, from) {
  return steps.map(s => ({ ...s, _from: s._from ? `${from} > ${s._from}` : from }));
}
...
function expandSteps(db, steps, callerHostname, visited) {
  ...
  for (const step of steps) {
    if (step.action === 'repeat') {
      // A repeat block keeps its structure (it loops at run time); only its
      // inner steps are expanded.
      out.push({ ...step, steps: expandSteps(db, step.steps || [], callerHostname, visited) });
    } else if (step.action === 'run_action') {
      ...
      nextVisited.add(key);
      out.push(...tagOrigin(expandSteps(db, subSteps, target.hostname, nextVisited), key));
      ...
      const nextVisited = new Set(visited);
      nextVisited.add(key);
      out.push(...tagOrigin(expandSteps(db, applyWith(subSteps, step.with), callerHostname, nextVisited), key));
    } else {
      out.push(step);
    }
  }
...
function checkUnresolvedRefs(db, steps, callerHostname) {
  ...
  for (const step of steps) {
    if (step.action === 'repeat') {
      problems.push(...checkUnresolvedRefs(db, step.steps || [], callerHostname));
    } else if (step.action === 'run_action' && step.ref) {
...
function stepsNeedHeaded(steps) {
  return steps.some(s => s.action === 'handoff' || (s.action === 'repeat' && stepsNeedHeaded(s.steps || [])));
}
