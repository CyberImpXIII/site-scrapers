...
const { autoDiagnose } = require('./probes');
...
async function captureFailureDiagnostics(page, meta, { error, consoleLog, networkFailures, rolling } = {}) {
  try {
      ...
      JSON.stringify(
        {
          ...
          // Which step was in flight when this failed (engine.js attaches
          // it to the error). Null for failures that aren't step failures
          // — a zero-result run, a timeout waiting for cards.
    ...
    // Run the standard probe sweep against the page as it actually is, right
    // now, before teardown. This is the point of the whole exercise: a
    // failure that explains itself costs one run, where re-running with
    // probes added costs two — and the second run may not even reproduce it
    // (a transient block, a page that loaded fine the next time).
    // Best-effort by construction: autoDiagnose never throws, and the outer
    // catch already covers a page too dead to evaluate against.
    try {
      const probes = await autoDiagnose(page);
      fs.writeFileSync(path.join(dir, 'diagnostics.json'), JSON.stringify(probes, null, 2));
    } catch {
      /* page unusable -- the screenshot and DOM are still worth keeping */
    }
